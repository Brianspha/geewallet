                            ...
                            yield!
                                ...
                                | Some(totalInUsd) ->
                                    seq {
                                        yield! statuses
                                        yield! totals
                                        yield sprintf "Total estimated value in USD: %s" (Formatting.DecimalAmountRounding CurrencyType.Fiat totalInUsd)
                                    }
                        }
                }
            else
                ...
                return seq {
                    for i = 0 to allAccounts.Count() - 1 do
                        ...
                        if matchFilter iterAccount then
                            yield! DisplayAccountStatus (i+1) iterAccount (NotFresh NotAvailable)
                }
            }
