...
module ServerManager =
    ...
    let UpdateServersFile () =
        ...
        for KeyValue(currency,servers) in baseLineServers do
            Console.WriteLine (SPrintF2 "%i %A servers from baseline JSON file" (servers.Count()) currency)

            match currency with
            | Currency.BTC ->
                Console.WriteLine (SPrintF1 "%i BTC servers from electrum repository" (electrumBtcServers.Count()))
                Console.WriteLine (SPrintF1 "%i BTC servers from bitcoin-eye" (eyeBtcServers.Count()))
            | Currency.LTC ->
                Console.WriteLine (SPrintF1 "%i LTC servers from electrum repository" (electrumLtcServers.Count()))
                Console.WriteLine (SPrintF1 "%i LTC servers from bitcoin-eye" (eyeLtcServers.Count()))
            | _ ->
                ()
        ...
        for KeyValue(currency,servers) in filteredOutServers do
            Console.WriteLine (SPrintF2 "%i %A servers total" (servers.Count()) currency)
